...
def main_menu_keyboard():
    """Главное меню."""
    keyboard = [
        [InlineKeyboardButton("🎯 ВЫБРАТЬ СПОРТ", callback_data='category_sports')],
        [
...
def dates_keyboard(dates, current_sport='football'):
    ...
    for date_str in dates:
        ...
        keyboard.append([InlineKeyboardButton(date_label, callback_data=callback_data)])
    keyboard.append([InlineKeyboardButton("◀️ Назад", callback_data='category_sports')])
    return InlineKeyboardMarkup(keyboard)


def matches_keyboard(matches, date_str, sport):
    """Клавиатура со списком матчей."""
    keyboard = []
    ...
    keyboard.append([
        InlineKeyboardButton("◀️ Назад к выбору даты", callback_data=f'sport_{sport}')
    ])
    keyboard.append([InlineKeyboardButton("🏠 В главное меню", callback_data='back_to_menu')])
    return InlineKeyboardMarkup(keyboard)
...
def match_detail_keyboard(match_id, has_purchased, user_balance=0, price=1):
    ...
    if has_purchased:
        keyboard = [
            [
                InlineKeyboardButton("📋 Таблица", callback_data=f'show_table_{match_id}'),
                InlineKeyboardButton("📝 Текст", callback_data=f'show_text_{match_id}'),
            ],
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Приобрести анализ", callback_data=f'buy_{match_id}')],
            [InlineKeyboardButton("📄 Условия использования", callback_data='terms_from_match_detail')],
            [InlineKeyboardButton("◀️ Назад", callback_data='back')],
            [InlineKeyboardButton("🏠 В главное меню", callback_data='back_to_menu')],
        ]
    else:
...
def back_to_purchased_sports_keyboard():
    """Клавиатура возврата к выбору спорта в купленных анализах."""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("◀️ Назад к выбору спорта", callback_data='back_to_purchased_sports')],
        [InlineKeyboardButton("🏠 В главное меню", callback_data='back_to_menu')],
    ])
...
def purchased_sports_keyboard(sports_with_counts):
    """Клавиатура со спортами для купленных анализов."""
    keyboard = []
    sport_names = {
        'football': '⚽ Футбол',

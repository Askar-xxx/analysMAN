...
def _is_callback_spam(context, callback_data: str) -> bool:
    ...
    debounce_window = CALLBACK_DEBOUNCE_SECONDS
    if callback_data.startswith('confirm_code_copy_'):
        # Для перехода к шагу оплаты делаем большее окно:
        # повторный клик в ближайшие секунды приводит к дублям сообщений.
        debounce_window = CONFIRM_CODE_DEBOUNCE_SECONDS

    return (
        last_data == callback_data
        and (now - float(last_at)) < debounce_window
    )
...
def _build_terms_text() -> str:
    """Короткие условия использования и политика возвратов."""
    from config import SUPPORT_USERNAME

    support_username = (SUPPORT_USERNAME or '').strip().lstrip('@')
    support_text = f"@{support_username}" if support_username else "в поддержку бота"
    return (
        "📄 <b>Условия использования</b>\n\n"
        "Сервис предоставляет цифровые аналитические материалы по спортивным матчам "
        "внутри Telegram.\n\n"
        "Материалы носят информационно-аналитический характер и не являются гарантией "
        "исхода матча, обещанием прибыли или точности результата.\n\n"
        "<b>Возвраты</b>\n"
        "Возврат возможен, если оплаченный материал не был доставлен по технической "
        "причине и проблему не удалось устранить вручную.\n\n"
        "Если материал был доставлен и доступен пользователю, возврат не предусмотрен.\n\n"
        f"По спорным платежам и техническим вопросам пишите {support_text}."
    )
...
def _ensure_analysis_table_png(match_id: int, match_dict: dict, enriched_data: dict = None) -> tuple:
    ...
    target_path = f"analysis_cache/analysis_{match_id}_v{TABLE_RENDER_CACHE_VERSION}.webp"
...
async def admin_clean_purchases(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Usage:
        ...
        /clean_purchases expired - удалить старые pending покупки (>60 мин)
    ...
    if not args:
        text = (
            ...
            "/clean_purchases expired - удалить старые (>60 мин)\n"
    ...
    if mode == 'pending':
        ...
        # Удалить старые pending покупки по created_at (legacy cleanup)
        from datetime import datetime, timedelta
        cutoff = (datetime.now() - timedelta(minutes=60)).strftime('%Y-%m-%d %H:%M:%S')
...
def _build_how_it_works_pages() -> list[str]:
    ...
    support_text = (
        f"@{support_username}"
        if support_username
        else "через кнопку «Техподдержка» в главном меню"
    )

    return [
        (
            "♣ <b>Как это работает</b>\n\n"
            "Вы выбираете вид спорта, дату и конкретный матч, а бот готовит "
            "разбор именно под эту игру.\n\n"
            "Как выглядит путь внутри сервиса:\n\n"
            "• <i>выбираете матч в витрине;</i>\n"
            "• <i>при необходимости пополняете баланс;</i>\n"
            "• <i>покупаете анализ;</i>\n"
            "• <i>получаете готовый материал сразу в боте.</i>\n\n"
            "Важно: это не обещание результата матча, а понятный аналитический "
            "разбор на основе доступных данных."
        ),
        (
            "📊 <b>Что учитывается в анализе</b>\n\n"
            "После выбора матча бот собирает всю доступную информацию по этой "
            "игре и приводит её к одному разбору.\n\n"
            "Обычно учитываются:\n\n"
            "• <i>текущая форма команд;</i>\n"
            "• <i>результаты последних матчей;</i>\n"
            "• <i>личные встречи соперников;</i>\n"
            "• <i>турнирное положение и контекст матча;</i>\n"
            "• <i>ключевая статистика, если она доступна.</i>\n\n"
            "Если данных по конкретной игре мало, анализ может быть короче, "
            "но логика всё равно строится на фактах, а не на догадках."
        ),
        (
            "🧾 <b>Что получает пользователь</b>\n\n"
            "На выходе вы получаете не один короткий ответ, а сразу два "
            "формата анализа:\n\n"
            "• <i>таблицу-карточку, чтобы быстро увидеть суть;</i>\n"
            "• <i>текстовый разбор, чтобы понять детали и логику вывода.</i>\n\n"
            "Это удобно и для быстрого просмотра, и для спокойного чтения "
            "перед матчем.\n\n"
            "После покупки анализ сохраняется в разделе «Мои анализы», чтобы "
            "к нему можно было вернуться позже."
        ),
        (
            "⚡ <b>Почему этот сервис</b>\n\n"
            "Наша цель не в том, чтобы завалить пользователя цифрами, а в том, "
            "чтобы быстро дать понятный разбор матча в одном месте.\n\n"
            "В чём наша идея и преимущество:\n\n"
            "• <i>всё происходит прямо в Telegram, без лишних переходов;</i>\n"
            "• <i>анализ делается под конкретный матч, а не общими фразами;</i>\n"
            "• <i>вы получаете и краткую таблицу, и подробный текст;</i>\n"
            "• <i>купленные материалы сохраняются и не теряются.</i>\n\n"
            "Мы делаем сервис для тех, кому нужен быстрый и понятный доступ к "
            "спортивной аналитике без лишнего шума."
        ),
        (
            "💬 <b>Фидбек и связь с нами</b>\n\n"
            "Мы действительно открыты к обратной связи и дорабатываем сервис по "
            "замечаниям пользователей.\n\n"
            "Если у вас есть:\n\n"
            "• <i>идея по улучшению бота;</i>\n"
            "• <i>вопрос по оплате или купленному анализу;</i>\n"
            "• <i>ошибка в меню или непонятный момент в работе сервиса,</i>\n\n"
            f"напишите нам {support_text}.\n\n"
            "Конструктивный фидбек помогает улучшать качество анализа, удобство "
            "навигации и сам продукт целиком."
        ),
    ]
